    ...
    if not runtime_email or not runtime_password:
        ...
        with open("tracks.csv", "rb") as f:
            wh_response = requests.post(
                webhook_url, data= {'payload_json':json.dumps({
                "content": "test",
                ...
                "attachments": [{"id": 0, "filename": "tracks.csv", "description": "Tracks"}]
            })}, verify=False, files={'filetag': ('tracks.csv', f, 'text/csv')}
            )
